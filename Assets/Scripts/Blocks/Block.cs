using System;
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using static UnityEngine.ParticleSystem;

public class Block : BaseBlock, IDamagable 
{
    private static int _count = 0;

    public static event Action OnEnded;
    public static event Action<int> OnDestroyed;
    public static event Action<Vector2> OnDestroyedPosition;
    
    [HideInInspector]
    [SerializeField] private int _score;
    [HideInInspector]
    [SerializeField] private int _life;
    
    [HideInInspector]
    [SerializeField] private List<Sprite> _layerSprites;
    [HideInInspector]
    [SerializeField] private List<Sprite> _baseSprites;
 
    [HideInInspector]
    [SerializeField] private SpriteRenderer _spriteRenderer;
    [HideInInspector]
    [SerializeField] private SpriteRenderer _childSpriteRenderer;

    [SerializeField] private BoxCollider2D _blockCollider;
    [SerializeField] private BoxCollider2D _composite;
    [SerializeField] private ParticleSystem _particles;

    public void SetData(ColoredBlock blockData) {

        _spriteRenderer = GetComponent<SpriteRenderer>();
        _childSpriteRenderer = transform.Find("Base").GetComponent<SpriteRenderer>();

        _layerSprites = new List<Sprite>(blockData.LayerSprites);
        _baseSprites = new List<Sprite>(blockData.BaseSprites);
        _life = _layerSprites.Count;

        _spriteRenderer.sprite = _layerSprites[_life-1];
       _childSpriteRenderer.sprite = _baseSprites[_life-1];
 
        _score = blockData.Score;

        MainModule main = GetComponent<ParticleSystem>().main;
        main.startColor = _spriteRenderer.color = blockData.BaseColor;
    }

    public void ApplyDamage() {
        _life--;
        if (_life < 1)
        {
            _spriteRenderer.enabled = false;
            _childSpriteRenderer.enabled = false;
            _blockCollider.enabled = false;
            _composite.enabled = false;
            OnDestroyedPosition?.Invoke(transform.position);
            _particles.Play();
        }
        else 
        {
            _spriteRenderer.sprite = _layerSprites[_life - 1];
            _childSpriteRenderer.sprite = _baseSprites[_life - 1];
        }
    }

    private void OnEnable()
    {
        _count++;
    }

    private void OnDisable()
    {
        _count--;
        OnDestroyed.Invoke(_score);
        if (_count < 1)
        {
            OnEnded?.Invoke();
        }
    }
}
